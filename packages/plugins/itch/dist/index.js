...
const exe = async (command, shouldLog = true) => {
    const log = [];
    const logger = utils_1.createScopedLogger('itch', {
        interactive: true,
    });
    return new Promise((resolve) => {
        const npmstart = child_process_1.exec(command);
        if (npmstart && npmstart.stdout && npmstart.stderr) {
            ...
            npmstart.on('exit', (code) => {
                resolve({
                    code: (code !== null && code !== void 0 ? code : '').toString(),
                    log,
                });
            });
        }
...
exports.default = {
    ...
    run: async function run({ workingDirectory, taskSettings }) {
        ...
        const { project, directories } = taskSettings;
        if (!project) {
